...
const Search = () => {
  ...
  const deleteListHandler = async (appointment_id) => {
    const resFromServer = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/api/delList`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const userOrAdmin = useCallback(async () => {
    const resFromServer = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/api/logedInUser`,
      {
        method: "GET",
        credentials: "include",
        headers: {
      ...
      <div className="searchResults innerList">
        {searchedUser.map((singleUser, i) => {
          return state && singleUser.user ? (
            <div className="userImg" key={singleUser._id}>
              <img src={userImg} alt="userImage" id="" />
              <div className="listText">
                <p className="cp userEmail">
                  {" "}
                  {singleUser.user.email
                    ? singleUser.user.email
                    : "loading"}{" "}
                </p>
                <p className="cp userName">
                  {singleUser.user.name ? singleUser.user.name : "loading name"}
                </p>
