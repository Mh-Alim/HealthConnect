...
const EditProfile = () => {
  ...
  const handleClick = async (e) => {
    try {
      ...
      console.log(name, phone, address, height, weight);
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND_URL}/api/editProfile`,
        {
          method: "POST",
          credentials: "include",
          headers: {
      ...
      console.log(
        " error in editProfile component handleClick function and error is " +
          err
      );
    }
  };
      ...
      >
        ...
        <MDBCard id="signup_card" className="m-5">
          <MDBCardBody className="px-5">
            ...
            <p className="ml-4">
              Go to <NavLink to="/profile">Previous</NavLink> page
            </p>
          </MDBCardBody>
        </MDBCard>
      </MDBContainer>
